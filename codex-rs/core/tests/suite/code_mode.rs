...
fn assert_repeated_char_output(output: &str, ch: char, len: usize) {
    assert_eq!(output.len(), len);
    assert!(
        output.chars().all(|actual| actual == ch),
        "output should contain only {ch:?}"
    );
}
...
"#,
    ...
    assert_regex_match(
        r"(?s)^Total output lines: 1\n\nA+…\d+ tokens truncated…A+$",
        text_item(
            &custom_tool_output_items(&second_mock.single_request(), "call-1"),
            /*index*/ 1,
        ),
