    ...
    fn process_item(&self, item: &ResponseItem, policy: TruncationPolicy) -> ResponseItem {
        ...
        match item {
            ...
            ResponseItem::CustomToolCallOutput {
                ...
                let is_code_mode_exec_output = name.as_deref() == Some(CODE_MODE_EXEC_TOOL_NAME)
                    || self.items.iter().rev().any(|item| {
                        matches!(
                            ...
                            ResponseItem::CustomToolCall {
                                ...
                                name,
                                ..
                            } if existing_call_id == call_id && name == CODE_MODE_EXEC_TOOL_NAME
                        )
                    });
                let output = if is_code_mode_exec_output {
                    output.clone()
                } else {
