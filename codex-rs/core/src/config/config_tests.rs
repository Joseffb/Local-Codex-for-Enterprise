...
use codex_model_provider_info::DOCKER_MODEL_RUNNER_DEFAULT_MODEL;
...
"#,
    ...
    assert_eq!(
        ...
        Some(DOCKER_MODEL_RUNNER_DEFAULT_MODEL)
    );
    assert!(
...
#[tokio::test]
async fn default_config_uses_docker_model_runner_without_openai_auth() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    ...
    assert_eq!(config.model_provider_id, DOCKER_MODEL_RUNNER_PROVIDER_ID);
    assert_eq!(
        config.model.as_deref(),
        Some(DOCKER_MODEL_RUNNER_DEFAULT_MODEL)
    );
...
#[tokio::test]
async fn explicit_model_and_provider_override_docker_defaults() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-5.4-codex"));
    assert_eq!(config.model_provider.wire_api, WireApi::Responses);
    assert!(config.model_provider.requires_openai_auth);
