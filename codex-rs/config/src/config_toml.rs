...
use codex_model_provider_info::DOCKER_MODEL_GATEWAY_PROVIDER_ID;
use codex_model_provider_info::DOCKER_MODEL_RUNNER_PROVIDER_ID;
...
pub struct ConfigToml {
    ...
    /// Whether Codex may prompt to configure Docker MCP Toolkit automatically.
...
mod tests {
    ...
    #[test]
    fn docker_mcp_auto_configure_deserializes() {
        let config: ConfigToml = toml::from_str("docker_mcp_auto_configure = false")
            .expect("docker MCP opt-out should deserialize");

        assert_eq!(config.docker_mcp_auto_configure, Some(false));
    }

    #[test]
    fn docker_model_provider_ids_are_reserved() {
        let err = toml::from_str::<ConfigToml>(
            r#"
[model_providers.docker-model-runner]
name = "Shadow Docker"
base_url = "http://example.com/v1"
            "#,
        )
        .expect_err("Docker Model Runner provider ID should be reserved");

        assert!(err.to_string().contains("docker-model-runner"));
    }
}
