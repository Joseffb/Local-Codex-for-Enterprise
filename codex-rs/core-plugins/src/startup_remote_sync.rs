...
pub(crate) fn start_startup_remote_plugin_sync_once(
    ...
    tokio::spawn(async move {
        ...
        let Some(auth) = auth_manager
            .auth()
            .await
            .filter(codex_login::CodexAuth::uses_codex_backend)
        else {
            info!("skipping startup remote plugin sync because ChatGPT auth is not configured");
            return;
        };
        match manager
            .sync_plugins_from_remote(&config, Some(&auth), /*additive_only*/ true)
            .await
        {
            Ok(sync_result) => {
                info!(
