...
async fn chatgpt_backend_auth(auth_manager: &AuthManager) -> Option<CodexAuth> {
    let auth = auth_manager.auth().await?;
    auth.uses_codex_backend().then_some(auth)
}

async fn warm_remote_plugin_state_if_authenticated(
    manager: Arc<PluginsManager>,
    config: PluginsConfigInput,
    auth_manager: Arc<AuthManager>,
    on_effective_plugins_changed: Option<Arc<dyn Fn() + Send + Sync + 'static>>,
) {
    let Some(auth) = chatgpt_backend_auth(&auth_manager).await else {
        return;
    };
...
async fn warm_featured_plugin_ids_cache_if_authenticated(
    manager: Arc<PluginsManager>,
    config: PluginsConfigInput,
    auth_manager: Arc<AuthManager>,
) {
    ...
    pub fn maybe_start_plugin_startup_tasks_for_config(
        ...
        if config.plugins_enabled {
            ...
            tokio::spawn(async move {
                warm_remote_plugin_state_if_authenticated(
                    manager,
                    config_for_remote_sync,
                    auth_manager_for_remote_sync,
                    on_effective_plugins_changed,
                )
                .await;
            });
            ...
            let manager = Arc::clone(self);
            tokio::spawn(async move {
                warm_featured_plugin_ids_cache_if_authenticated(manager, config, auth_manager)
                    .await;
            });
        }
    }
